...
def FEDAVG(args, test_dataset, train_dataset, user_groups, N_us, file_path, transmit_power, bitwidths, prune_rates, computing_resources, I_us, h_us):
    ...
    T_AVG = fl_utils.calculate_T(transmit_power,bitwidths,prune_rates,computing_resources,N_us,args.B_u,h_us,args.N0,I_us,args.V,xis,args.c0,args.s,args.Rate)
    ...
    for epo in range(args.num_epoch):
        ...
        # 服务器操作（梯度聚合+更新）
        agg_grads = server.aggregate(w=local_weights, alpha=alpha.to(device))   # 梯度聚合   
        server.update(avg_weights=agg_grads)                                                                                    # 梯度更新
